...
![12.2.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b291d68b2faa477b91c615751c49ba33~tplv-k3u1fbpfcp-watermark.awebp)

**3、Swift和OC有什么区别？**

Swift和OC的区别有很多，这里简要总结这几条：

![++.png.PNG](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/9577cb14d49b4abc924d8d53abcf6101~tplv-k3u1fbpfcp-watermark.awebp)
...
\####OC语法 **1、Block是如何实现的？Block对应的数据结构是什么样子的？__block的作用是什么？它对应的数据结构又是什么样子的？**
...
![12.3.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/cedd7e163d334201950ee7b6734fe2b8~tplv-k3u1fbpfcp-watermark.awebp)
...
![12.4.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4681a94698fc4630a2daec1a135bfb3d~tplv-k3u1fbpfcp-watermark.awebp)

看这张图我们可以知道，它是UIKit和AppKit的底层实现，位于Metal、Core Graphics和GPU之上之上。

苹果官方文档：About Core Animation

\####引用计数 **1、ARC方案的原理是什么？它是在什么时候做的隐式添加release操作？**
...
消息转发的时候，如果是实例方法会走resolveInstanceMethod:，如果是类方法会走resolveClassMethod:，它们的返回值都是Bool，需要我们确定是否进行转发。 如果第一步返回YES，确定转发就会进到下个方法forwardingTargetForSelector，这个方法需要我们指定一个被用receiver。 methodSignatureForSelector用于指定方法签名，forwardInvocation用于处理Invocation，进行完整转发。 如果消息转发也没有处理即为无法处理，会调用doesNotRecognizeSelector，引发崩溃。
...
![12.5.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/eb5d1c16edd948799d01354004be9a89~tplv-k3u1fbpfcp-watermark.awebp)
...
hook系统方法进行方法交换。 了解一个类（闭源）的私有属性和方法。 关联对象，实现添加分类属性的功能。 修改isa指针，自定义KVO。

\####Runloop **1、讲一下对Runloop的理解？** Runloop就是一个运行循环，它保证了在没有任务的时候线程不退出，有任务的时候即使响应。Runloop跟线程，事件响应，手势识别，页面更新，定时器都有着紧密联系。
...
![12.6.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f51227be20544b37a3c717c75dc4b3c2~tplv-k3u1fbpfcp-watermark.awebp)
...
\####计算机相关 **1、项目编译的流程是什么？手机上的应用程序自点击图标开始到首屏内容展示都经历了哪些步骤？**
...
![12.7.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/b998a18543964f1daabfd6c79dddaea6~tplv-k3u1fbpfcp-watermark.awebp) 这个算法在SDWebImage和Kingfisher等需要处理缓存的库中都有实现。
...
![12.8.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/93898ff40fd44cce84e1beddee24c105~tplv-k3u1fbpfcp-watermark.awebp) UIKit是基于CoreAnimation渲染的，而Flutter并没有用到它，而是自己基于C++实现了一套渲染框架。
...
\####数据结构与算法 **1、什么是Hash表，什么是Hash碰撞，解决Hash碰撞有什么方法？**
...
![12.9.png.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/bd999676298f4ee38b98944cf1dfe1b9~tplv-k3u1fbpfcp-watermark.awebp) 二叉树的遍历有三种方式，对于上面这棵二叉树，他们的遍历结果为：
...
作者：iOS南方
链接：https://juejin.cn/post/6990228005293408270
来源：稀土掘金
